...
function filterStudentsByGrade(students, grade) {
  ...
  let filtered = students.filter((student) => student.grade === grade)
...
function logArrayElements(arr) {
  ...
  for (let key in arr){
    console.log(`Елeмент ${key}: ${arr[key]}`)
  }
...
function compareArrays(arr1, arr2) {
  ...
  console.assert(JSON.stringify(sortedArr1) === JSON.stringify(sortedArr2), "масиви не однакові")
...
function displayGroupedInfo(students) {
  for (let st of students) {
    console.group(`Студент: ${st.name}`)
    console.info(`Вік: ${st.age}`)
    console.info(`Предмет: ${st.subject}`)
    console.groupEnd(`Студент: ${st.name}`)
  }
...
function calculateTotalPrice(products) {
  ...
  console.timeEnd('Час обчислення')
...
function countVowelsAndConsonants(word) {
  ...
  for (let letter of word.toLowerCase()) {
    st.includes(letter) ? Gcount++ : Pcount++
  }
  console.log(`Голосних: ${Gcount}`)
  console.log(`Приголосних: ${Pcount}`)
